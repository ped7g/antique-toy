...
> **Мультиколор і спірна пам'ять (48K/128K Spectrum).** Техніка LDPUSH записує напряму в екранну пам'ять через SP, тож кожен PUSH потрапляє в спірні адреси ($4000--$5AFF). На стандартному Spectrum 48K або 128K ULA вставляє такти очікування під час активного періоду відображення, додаючи ~1 T на кожен PUSH. Що ще критичніше, потактові перегони з променем --- прив'язка зміни атрибутів до конкретних рядків розгортки --- стає ненадійною, бо затримки спірної пам'яті зсувають таймінг на змінні величини залежно від фази піксельного годинника ULA. Рушій GLUF DenisGrachev'а розроблявся і тестувався на Pentagon, де спірної пам'яті немає. **На не-Pentagon машинах** підхід Ringo з двома екранами (перемикання сторінок через порт $7FFD між групами рядків розгортки) надійніший: він не потребує потактової точності, бо перемикання сторінки набуває чинності на наступній межі рядка розгортки незалежно від моменту виконання інструкції OUT. Розглядай LDPUSH-мультиколор як Pentagon-орієнтовану техніку, якщо не можеш перевірити таймінг на цільовому обладнанні. Див. Розділ 15.2 для моделі спірної пам'яті.
