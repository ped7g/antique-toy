...
> **Скролінг на не-Pentagon машинах.** Бюджет вище розрахований для тайміну Pentagon. На стандартному Spectrum 48K або 128K ланцюжок RL записує у відеопам'ять у спірній ділянці ($4000--$7FFF), і кожне звернення під час активного періоду відображення спричиняє такти очікування. Очікуй зростання вартості на рядок з ~704 T до ~850 T --- приблизно **20% накладних витрат**. Тіньовий екран (сторінка 7 на 128K) теж у спірній пам'яті, тому подвійна буферизація не рятує від штрафу. Передачі буфер-на-екран (LDIR або через PUSH) впираються в ту саму стіну: *джерело* може бути в неспірній пам'яті вище $8000, але *призначення* --- завжди відеопам'ять. **Пом'якшення:** зменши область скролінгу, використовуй скролінг по знакорядах де можливо, або синхронізуй передачу з часом бордюру. Див. Розділ 15.2 для таблиць тайміну спірної пам'яті та стратегій планування.
