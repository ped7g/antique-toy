...
![Маска AND очищує область спрайта на шахматному фоні, потім OR друкує графіку. В масці стоять 0 там, де будуть пікселі спрайта (AND їх стирає), і 1 в решті місць (AND зберігає фон).](../../illustrations/output/ch16_sprite_masking.png)
...
> **Спірна пам'ять і рендеринг спрайтів (48K/128K Spectrum).** Бюджет вище розрахований для тайміну Pentagon без тактів очікування. На стандартному Spectrum 48K або 128K записи у відеопам'ять ($4000--$5AFF для пікселів, $5800--$5AFF для атрибутів) потрапляють у спірну пам'ять --- ULA краде такти під час активного періоду відображення. Кожен `PUSH` у стековому спрайті або кожен `LD (HL),A` у маскованому спрайті коштує приблизно 1 зайвий такт на звернення до цієї ділянки, сумарно ~1,3 T на 2-байтовий PUSH. Для системи з 8 спрайтів це може додати 3 000--5 000 тактів накладних витрат. **Практичне правило:** на не-Pentagon машинах плануй малювання спрайтів на час бордюру (верхній/нижній бордюр дають ~14 000 тактів без спору) або закладай бюджет на 2 кадри з ширшими запасами. Див. Розділ 15.2 для повного розбору патернів спірної пам'яті ULA та стратегій пом'якшення.
